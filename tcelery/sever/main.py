...
from tcelery.app import Application
from tcelery import setup_nonblocking_producer
from celery.loaders.base import BaseLoader
from task.task import app, update_app_config, get_openerp
...
class TCeleryCommand(Command):
    def run_from_argv(self, prog_name, argv=None):
        ...
        http_server = httpserver.HTTPServer(Application(celery_app=app))
        http_server.listen(options.port, options.address)

        bloader = BaseLoader(app)
